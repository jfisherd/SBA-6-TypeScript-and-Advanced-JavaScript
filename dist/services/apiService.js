...
export const contactApi = async () => {
    ...
    try {
        ...
        for (let i = 0; i < objectArray.length; i++) {
            ...
            objectArray[i].id, objectArray[i].title, objectArray[i].description, objectArray[i].category, objectArray[i].price, objectArray[i].discountPercentage, objectArray[i].sku);
        }
    }
    catch (error) {
